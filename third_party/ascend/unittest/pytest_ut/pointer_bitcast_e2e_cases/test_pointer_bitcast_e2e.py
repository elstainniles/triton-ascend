...
@triton.jit
def _load_u32_from_u8_tensor_offsets(src, out, n_elements, BASE: tl.constexpr,
                                     BLOCK: tl.constexpr):
    lanes = tl.arange(0, BLOCK)
    mask = lanes < n_elements
    byte_offsets = BASE + lanes * 4
    u32_ptrs = (src + byte_offsets).to(tl.pointer_type(tl.uint32, 1),
                                       bitcast=True)
...
@triton.jit
def _load_bf16_from_u8_multi_addptr(src, out, block_idx, token_pos,
                                    block_stride, N: tl.constexpr,
...
@triton.jit
def _load_bf16_from_u8_inside_loop(src, out, count, NOPE_DIM: tl.constexpr,
                                   TOKEN_BYTES: tl.constexpr,
...
def test_load_uint32_scale_words_from_uint8_tensor_offsets():
    ...
    host = torch.zeros(base + block * 4, dtype=torch.uint8)
    _write_u32_le(host, [base + idx * 4 for idx in range(n_words)], values)

    src = host.npu()
    out = torch.empty((block,), device="npu", dtype=torch.int32)

    _load_u32_from_u8_tensor_offsets[(1,)](src, out, n_words, BASE=base,
                                           BLOCK=block)
    _sync_npu()

    expected = torch.zeros((block,), dtype=torch.int32)
    expected[:n_words] = torch.tensor(values, dtype=torch.int32)
...
def test_load_bf16_rope_region_from_uint8_multi_addptr():
    ...
    host = torch.zeros(base + n_values * 2 + 64, dtype=torch.uint8)
...
def test_load_bf16_rope_region_from_uint8_inside_dynamic_loop():
    ...
    for token_idx in range(token_count):
        values = [float(token_idx * rope_dim + idx + 1)
                  for idx in range(rope_dim)]
        expected_values.extend(values)
        _write_bf16_le(host, token_idx * token_bytes + nope_dim, values)

    src = host.npu()
    out = torch.empty((token_count * rope_dim,),
                      device="npu",
