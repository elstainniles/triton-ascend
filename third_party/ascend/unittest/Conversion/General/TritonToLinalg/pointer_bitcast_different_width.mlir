...
// CHECK: arith.divsi
// CHECK: builtin.unrealized_conversion_cast {{.*}} : memref<?xi8> to memref<?xbf16>
// CHECK: memref.reinterpret_cast {{.*}} : memref<?xbf16> to memref<1xbf16
...
// CHECK: builtin.unrealized_conversion_cast {{.*}} : memref<?xi8> to memref<?xi32>
// CHECK: memref.reinterpret_cast {{.*}} : memref<?xi32>
module attributes {hacc.target = #hacc.target<"Ascend910B2">} {
...
// CHECK: arith.muli
// CHECK: builtin.unrealized_conversion_cast {{.*}} : memref<?xi32> to memref<?xi8>
module attributes {hacc.target = #hacc.target<"Ascend910B2">} {
